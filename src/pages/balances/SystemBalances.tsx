    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        {data?.data.map((balance) => {
          const isFeeCollection = balance.accType === "FEE_COLLECTION";
          return (
            <div
              key={balance.accType}
              className={`card p-6 relative overflow-hidden group ${isFeeCollection ? "cursor-pointer hover:border-indigo-500/50 transition-colors" : ""}`}
              onClick={() => isFeeCollection && navigate("/balances/fees")}
            >
              <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                <svg className={`w-16 h-16 ${isFeeCollection ? "text-indigo-500" : "text-primary-500"}`} fill="currentColor" viewBox="0 0 24 24">
